...
@dataclasses.dataclass
class SmokeLevel(TypeMapping):
    value: str

    @classmethod
    def from_tuple(cls, tup: Tuple[int]):
        return cls(value="High" if tup[0] / 100 > 0.8 else "Low")
    ...
    @staticmethod
    def set_value_type_info():
        return set_type_info(SmokeLevel.type_mapping())


@dataclasses.dataclass
class Alert(TypeMapping):
    message: str
    timestamp: int
    temperature: float
    ...
    @staticmethod
    def type_mapping():
        return {"message": Types.STRING(), "timestamp": Types.LONG(), "temperature": Types.DOUBLE()}

    @staticmethod
    def set_value_type_info():
        return set_type_info(Alert.type_mapping())
